...
func main() {
	...
	// Initial connection
	if err := connMgr.Connect(ctx, *durable); err != nil {
...
func (cm *ConnectionManager) Connect(ctx context.Context, durable bool) error {
	cm.mu.Lock()
	defer cm.mu.Unlock()
	...
	for i := 0; i < len(cm.servers); i++ {
		...
		// Determine sender settle mode based on durability
		var settleMode *amqp.SenderSettleMode
		if durable {
			// For durable messages, use Unsettled for reliability
			mode := amqp.SenderSettleModeUnsettled
			settleMode = &mode
			log.Printf("Using SenderSettleModeUnsettled for durable messages")
		} else {
			// For non-durable messages, use Settled for maximum performance
			mode := amqp.SenderSettleModeSettled
			settleMode = &mode
			log.Printf("Using SenderSettleModeSettled for non-durable messages (fire-and-forget)")
		}

		// Create sender with optimized settlement mode
		sender, err := session.NewSender(ctx, cm.queueName, &amqp.SenderOptions{
...
func (cm *ConnectionManager) Reconnect(ctx context.Context, durable bool) error {
	cm.mu.Lock()
		...
		for _, msgWithHeaders := range messages {
			...
			default:
				...
				if len(msgWithHeaders.Headers) > 0 {
					...
					log.Printf("Set ApplicationProperties: %v", appProps)
				}

				// Add increasing message number
				appProps["messageNumber"] = messageNum
				messageNum++

				// Create message
				message := amqp.NewMessage(payload)
				message.ApplicationProperties = appProps
				...
				for {
					...
					default:
						...
						if sender == nil {
							...
							if err := connMgr.Reconnect(ctx, durable); err != nil {
...
func producer(ctx context.Context, connMgr *ConnectionManager, msgSize int, durable bool, messagesSent *atomic.Uint64) {
	...
	for {
		...
		default:
			...
			// Send message (non-blocking with ModeSettled, blocks with ModeUnsettled)
			err := sender.Send(ctx, message, nil)
			if err != nil {
				log.Printf("Failed to send message: %v, attempting reconnection...", err)
				if err := connMgr.Reconnect(ctx, durable); err != nil {
